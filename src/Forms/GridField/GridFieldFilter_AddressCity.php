...
/**
 * GridField component that renders a city dropdown filter for addresses.
 *
 * @package Clesson\Silverstripe\Contacts
 * @subpackage Forms\GridField
 */
class GridFieldFilter_AddressCity implements GridField_HTMLProvider, GridField_DataManipulator, GridField_ActionProvider
{
    use Injectable;

    /**
     * State key used to store the selected city.
     */
    private const STATE_KEY = 'GridFieldFilter_AddressCity';
    ...
    {
        if ($actionName !== 'filteraddressbycity') {
            return;
        }

        $state = $gridField->State;
        $fieldName = static::STATE_KEY;
        $state->$fieldName = $data['gridfield_address_city_filter'] ?? '';
    ...
    public function getHTMLFragments($gridField): array
    {
        ...
        $allLabel = _t(__CLASS__ . '.ALL_CITIES', 'All cities');
        $dropdownOptions = ['' => $allLabel] + $options;

        /** @var DropdownField $dropdownField */
        $dropdownField = DropdownField::create('gridfield_address_city_filter', '', $dropdownOptions);
        ...
        $actionField = GridField_FormAction::create(
            $gridField,
            'filteraddressbycity',
            _t(__CLASS__ . '.LABEL', 'City'),
            'filteraddressbycity',
            null
        );
        $actionField->setAttribute('style', 'display:none;');
        $actionField->addExtraClass('grid-field__address-city-filter-action');

        $dropdownField->setAttribute(
            'onchange',
            'this.closest(\'.grid-field\').querySelector(\'.grid-field__address-city-filter-action\').click();'
        );

        $html = '<div class="contacts-address-city-filter" style="display:inline-flex;align-items:center;gap:8px;padding:4px 0 8px;">'
    ...
    /**
     * Applies the city filter to the DataList.
     *
     * @param GridField $gridField
    ...
    public function getManipulatedData(GridField $gridField, SS_List $dataList): SS_List
    {
        ...
        return $dataList->filter('City', $selected);
    }
    ...
    /**
     * Builds the dropdown options from existing address cities.
     *
     * @return array<string, string>
     */
